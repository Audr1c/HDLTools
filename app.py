...
from sv_generator import generate_module_code, generate_testbench_code
...
def draw_highlighted_line(surface, font, line_text, x, y):
    import re
    # Match keywords, macros, identifiers, comments, and numbers
    # Group comments first to avoid highlighting keywords inside comments
    ...
    for word in words:
        ...
        elif word in ["module", "endmodule", "input", "output", "inout", "logic", "reg", "wire", "parameter", "localparam", "initial", "begin", "end", "always", "forever", "wait", "posedge", "negedge", "repeat", "timescale", "`timescale"]:
...
def main():
    ...
    # Active Panels / Tabs
    # Right panel can display: "diagram" or "code"
    right_tab = "diagram" 
    # Under "code", subtab can display: "module" or "testbench"
    ...
    sync_reset_cb = Checkbox((230, 95, 200, 26), "Synchronous Reset", initial_val=is_sync_reset)
    active_low_cb = Checkbox((20, 95, 200, 26), "Active Low Reset", initial_val=is_active_low_reset)
    ...
    def rebuild_port_rows():
        ...
        for idx, p in enumerate(ports):
            ...
            # Row position will be set in draw/event loop relative to scrolling
    ...
    btn_add_clk = Button((20, 130, 100, 26), "Add Clock", callback=add_clock_preset)
    btn_add_rst = Button((130, 130, 100, 26), "Add Reset", callback=add_reset_preset)
    
    btn_add_input = Button((230, 170, 110, 26), "+ Input", callback=add_input_port, is_accent=False)
    btn_add_output = Button((350, 170, 110, 26), "+ Output", callback=add_output_port, is_accent=False)
    
    # Scroll Areas
    ports_scroll = ScrollArea((20, 210, 460, 410))
    ...
    def update_code_cache():
        ...
        if code_subtab == "module":
            ...
            code_str = generate_testbench_code(module_name, ports, is_sync_reset, is_active_low_reset)
    ...
    def handle_save_testbench():
        path = select_save_file(f"tb_{module_name}.sv", is_tb=True)
        if path:
            try:
                code_str = generate_testbench_code(module_name, ports, is_sync_reset, is_active_low_reset)
                with open(path, 'w') as f:
                    f.write(code_str)
                toast.show(f"Saved testbench to {os.path.basename(path)}")
            except Exception as e:
                toast.show(f"Error saving: {e}", is_error=True)
        ...
        if path:
            try:
                ...
                if parsed_name:
                    ...
                    if rst_p:
                        # Auto set active low if ends with _n or _b
    ...
    while True:
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_TAB:
                    ...
                    if curr_idx != -1:
                        ...
                        # If the focused widget is inside the scroll area, scroll to it
                        # The port rows index can be derived:
                        # Index 0 is module name. Index 1, 2 is Row 0 (name, width), Index 3, 4 is Row 1, etc.
                        if next_idx > 0:
                            row_idx = (next_idx - 1) // 2
                            # Ensure visible in viewport
                            target_vy = row_idx * 36
                            viewport_h = ports_scroll.rect.height
                            # If row is below viewport or above viewport, scroll to it
            ...
            # Check collisions inside ports viewport
            mouse_pos = pygame.mouse.get_pos()
            in_scroll_viewport = ports_scroll.rect.collidepoint(mouse_pos)
            
            # Translate mouse position relative to scrolled surface
            offset_pos = (mouse_pos[0], mouse_pos[1] + ports_scroll.scroll_y) if in_scroll_viewport else (-1, -1)
            
            # Update inputs and handle events inside scroll viewport
            for idx, row in enumerate(port_rows):
                row.set_positions(20, 210 + idx * 36)
                # Only handle events if inside viewport
        ...
        # Update each port row logic
        for r in port_rows:
            r.update()
            
        # Update scroll area heights
        ports_scroll.virtual_height = len(ports) * 36
        
        # Check if ports list changed and update cache if typing is active
        # (This is triggered implicitly when port names/widths change)
        # To avoid heavy regeneration, we recalculate code preview on modification
        ...
        # Ports list divider
        pygame.draw.line(screen, theme.colors["sideBar.border"], (20, 162), (480, 162), 1)
        
        # Ports title
        lbl_ports = theme.fonts["body_bold"].render("Module Ports", True, theme.colors["editor.foreground"])
        screen.blit(lbl_ports, (20, 175))
        
        btn_add_input.draw(screen)
        btn_add_output.draw(screen)
        
        # Render scrollable list of ports
        def draw_ports_list(surface, scroll_y):
            # Draw rows
            for idx, r in enumerate(port_rows):
                # Calculate screen position inside clipping rect
                ry = 210 + idx * 36
                r.set_positions(20, ry)
                # Shift drawing position based on scroll offset
                # Since the scroll viewport clips at (20, 210, 460, 410)
                # we just shift the widgets when drawing by scroll_y
                
                # Create a sub-surface render or shift widgets rect
                # Let's adjust top-left, draw, then restore
        ...
        if right_tab == "diagram":
            ...
            def draw_code_viewport(surface, scroll_y):
                ...
                for idx, line in enumerate(generated_code_lines):
                    ...
                    # Culling (only draw if visible in ScrollArea rect)
